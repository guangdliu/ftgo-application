...
public class AnnotatedCreateOrderSagaTest {
  ...
  public void shouldCreateOrder() {
    ...
    andGiven().
        successReply().
    expect().
      command(new AuthorizeCommand(CONSUMER_ID, ORDER_ID, CHICKEN_VINDALOO_ORDER_TOTAL)). /// 4. Verify that it sends a CreateTicket message to kitchen service
  ...
  public void shouldRejectOrderDueToConsumerVerificationFailed() {
    ...
    expect().
        ...
        to(ConsumerServiceChannels.consumerServiceChannel).
    andGiven().
        failureReply(). /// 5 Send a failure reply indicating that consumer service rejected order
    expect().
        command(new RejectOrderCommand(ORDER_ID)).
        to(OrderServiceChannels.orderServiceChannel); /// 6. verify that the saga sends a RejectOrderCommand message to order service. 
  }
