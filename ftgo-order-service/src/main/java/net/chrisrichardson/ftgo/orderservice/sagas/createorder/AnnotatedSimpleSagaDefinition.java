...
public class SimpleSagaDefinition<Data> implements SagaDefinition<Data> {
  ...
  private StepToExecute<Data> nextStepToExecute(SagaExecutionState state, Data data) {
    ...
    /***
    @param state is current state. You have data and need to decide what the next step is. 
    The very first start state is a dummy step with its state.getCurrentlyExecuting() is -1. 
    
    All steps are saved in sagaSteps. A step can tell you two things:
    if it has an action or not
    If it has a compesating action or not.  
    
    There are four cases here: w/o action, w/o compensating. 
    We must find an action from sagaSteps, either a forward or backward. 
    We stop if we find one and we skip if we don't find any. 
