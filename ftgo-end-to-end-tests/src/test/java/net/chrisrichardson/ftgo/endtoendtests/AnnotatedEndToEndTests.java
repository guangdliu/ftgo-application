...
/***
The end-to-end test is quite similar to the implementation
of the component test discussed. These tests are written
using Gherkin and executed using Cucumber. The Gradle Docker Compose plugin runs 
the containers before the tests run. It takes around four to five minutes to start
the containers and run the tests.

Basically, Docker Compose starts different servers and services. 
Check  
build-and-test-all.sh
run-end-to-end-test.sh files.

in build-and-test-all.sh, you can see how Docker Compose starts these servers and dependent 
services after building the jar

but looks like create-revise-cancel.feature is not used anywhere. I guess create-revise-cancel.feature
is only used to describe the test case. It's not used to generate a test cases, e.g. the one (place-order.feature)
for OrderSeriveComponentTest and OrderServiceComponentTestStepDefinitions. 
OrderSeriveComponentTest uses place-order.feature to invoke different methods in OrderServiceComponentTestStepDefinitions
to set up a mocked environment (e.g. mocked accounting service by using sagaParticipantStubManager),
but here in end-to-end testing, we don't need these mocked services since they are all generated. So we don't need a similar 
class to OrderSeriveComponentTest and OrderServiceComponentTestStepDefinitions. 

Instead, EndToEndTests directly 

***/
public class AnnotatedEndToEndTests {
